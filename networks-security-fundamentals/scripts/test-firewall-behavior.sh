...
benchmark_action() {
    ...
    # Firewall politikası uygulanıyor

    echo -e "\n[*] Test Edilen Politika: ${target_action}"
    iptables -F
    # Firewall kuralı ekleniyor, DROP veya REJECT olarak
    if [ "${target_action}" == "REJECT" ]; then
        iptables -A INPUT -p tcp --dport "${PORT}" -j REJECT --reject-with tcp-reset
    else
        iptables -A INPUT -p tcp --dport "${PORT}" -j DROP
    fi
    # TCP bağlantısı test ediliyor ve yanıt süresi ölçülüyor
